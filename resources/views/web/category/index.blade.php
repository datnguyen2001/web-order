    ...
    <div class="box-content">
        ...
        <div class="box-list-content-category">
            ...
            <div class="box-content-category-right">
                <form @if(!$name && !$slug && $status ) action="{{route('category',$cate->slug)}}"  @elseif($name && !$slug && $status) action="{{url('danh-muc/'.$cate->slug.'/'.$cateTwo->slug)}}" @else action="{{url('danh-muc/'.$cate->slug.'/'.$cateTwo->slug.'/'.$cateThree->slug)}}" @endif method="GET" >
                <div class="line-menu-search-category custom-shadow">
                    ...
                    <div class="box-filter-price d-flex align-items-center mx-3 gap-2">
                        ...
                        <div class="box-number-search-menu">
                            <input type="text" placeholder="Giá thấp nhất" name="min_price" class="input-search-price-menu">
                            <span class="title-đ">đ</span>
                        </div>
                        <div style="width: .375rem;height: 1px;background-color: rgba(85, 85, 85, .5)"></div>
                        <div class="box-number-search-menu">
                            <input type="text" placeholder="Giá cao nhất" name="max_price" class="input-search-price-menu">
                            <span class="title-đ">đ</span>
                        </div>
                        <button  class="btn-search-price-menu">Tìm kiếm</button>
                        <a @if(!$name && !$slug && $status ) href="{{route('category',$cate->slug)}}"  @elseif($name && !$slug && $status) href="{{url('danh-muc/'.$cate->slug.'/'.$cateTwo->slug)}}" @else href="{{url('danh-muc/'.$cate->slug.'/'.$cateTwo->slug.'/'.$cateThree->slug)}}" @endif class="btn-delete-search-menu">
