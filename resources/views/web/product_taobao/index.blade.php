    ...
    <div class="w-100 bg-white">
        <div class="box-content-detail-sp">
            ...
            <div class="box-content-detail">
                ...
                @if(count($productAttribute) > 0 && count($productValue) > 0)
                    <div class="d-flex box-attribute mt-4">
                ...
                @elseif(count($productAttribute) === 0 && count($productValue) === 0)
                ...
                @elseif(count($productAttribute) === 0 && count($productValue) > 0)
                    <div class="d-flex align-items-center mt-2" id="quantity-no-attribute-yes-value" style="gap: 10px"
                         data-product-name="{{$data->name}}" data-value-name="{{$productValue[0]->name}}" data-attribute-name="{{null}}"
                         data-product-image="{{$data->src}}" data-product-value-image="{{$data->src}}"
                         data-attribute-chinese-price="{{$data->price}}" data-attribute-vietnamese-price="{{$data->price * $setting->exchange_rate}}">
                        <div class="box-quantity-fa">
                            <button class="btn-minus-plus" data-type="minus"><i class="fa-solid fa-minus"></i></button>
                ...
                <div class="swiper productSwiper">
                    <div class="swiper-wrapper">
                        @foreach($SimilarProducts as $similar)
                            <div class="swiper-slide">
                                <a href="{{route('taobao.detail-product',$similar->slug)}}" class="box-product-item">
...
@stop
@section('script_page')
    <script src="{{asset('assets/js/product_taobao.js')}}"></script>
    <script>
